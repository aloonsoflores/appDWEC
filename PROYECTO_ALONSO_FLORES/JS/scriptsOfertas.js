document.addEventListener("DOMContentLoaded", function () {
  ...
  function cargarYMostrarJuegosMesa() {
    $.getJSON("../JSON/productoJSON.json", function (datos) {
      ...
      juegosConDescuento = listaJuegosMesa.filter(
  ...
  function ordenarJuegos(criterio) {
    switch (criterio) {
      case "masVendidos":
        juegosConDescuento.sort((a, b) => b.nota - a.nota);
        break;
      case "precioAltoBajo":
        juegosConDescuento.sort((a, b) => b.precio - a.precio);
        break;
      case "precioBajoAlto":
        juegosConDescuento.sort((a, b) => a.precio - b.precio);
        break;
      case "nuevo":
        juegosConDescuento.sort(
          (a, b) => new Date(b.fecha) - new Date(a.fecha)
        );
        break;
      case "ordenAlfabetico":
        juegosConDescuento.sort((a, b) => a.nombre.localeCompare(b.nombre));
        break;
      default:
        break;
    }
  ...
  function actualizarSelectorPagina() {
    ...
    const numPaginas = Math.ceil(juegosConDescuento.length / juegosPorPagina);
    const numBotones = 5;
